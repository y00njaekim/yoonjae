...
- `setExceptionBreakpoints` is a separate request, not a launch argument, so launch.json cannot
  express it — send it from the command line during the session. Default `'default'` is the
  adapter's recommendation (debugpy: `uncaught`).
...
Each call replaces the whole set and needs an active session.
